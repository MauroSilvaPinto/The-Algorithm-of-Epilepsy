...
for i in range(n_iterations):
    ...
    elif(i>=7 and p_switch>0.45): # with 0.48 it increases gradually
        p_switch=p_switch-0.03  
        
    elif(i>=22 and i<=600 and p_switch<=0.45):
        p_switch=0.46+(math.cos(np.random.uniform(-math.pi, math.pi))*0.03)
        
    elif(i>600):
        p_switch=0.43+(math.cos(np.random.uniform(-math.pi, math.pi))*0.02)
